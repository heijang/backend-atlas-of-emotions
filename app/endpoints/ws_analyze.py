...
from app.voice_identifier import (
    compare_voice_with_memory,
    get_user_uid_by_user_id,
    load_user_voice_embedding_to_memory,
    save_user_voice_embedding_to_db,
)
            ...
            if msg.get("type") == "websocket.receive":
                if "text" in msg:
                    ...
                    with open(temp_path, "ab") as f:
                        f.write(chunk)
                    # 분기: 등록용/일반 오디오
                    if session_mode.get(sid) == "audio_data":
                        # 일반 오디오 처리 (실시간 STT)
